...
class Levelsys(commands.Cog):
      ...
      if not message.author.bot:
        ...
        if str(message.guild.id) in data:
          if str(message.author.id) in data[str(message.guild.id)]:
            ...
            if new_level > lvl:
              #channel = commands.get_channel(id = 985901809933623317)
              await message.channel.send(f"***{message.author.name}*** nice your level  {new_level} 🎉")
  ...
  async def id(self, ctx: commands.Context, user: Optional[discord.Member]):
